...
| **SiLU 활성함수** | ReLU는 음수 영역에서 완전히 0값으로 만들어 죽은 뉴런이 생깁니다.  SiLU는 음수 영역을 작게 살려둡니다. 많은 모델들이 채택한 활성화 함수입니다. |
...
CNN 저수준 특징(4족보행 동물, 색깔)부터 고수준 특징(귀의 방향)까지 선, 형태, 경계등을 학습하여 조건이 가장 많고 비슷한 객체를 더 높이 채첨하는 방식이란걸 유추할 수 있습니다
...
```
# C3, C4, C5 추출 (128/256/512, stride 8/16/32)
x = model.stem(x)
x = model.stage1(x)
x = model.stage2(x)
c3 = model.stage3(x)
